...
func show_panel():
	...
	# Animate in
	var tween = create_tween()
	tween.tween_property(control, "modulate:a", 1, 0.2)
	tween.parallel().tween_property(control, "scale", Vector2(1, 1), 0.2).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_BACK)
...
func _hide_panel():
	...
	tween.parallel().tween_property(control, "scale", Vector2(0.95, 0.95), 0.15)
